                                  ...
                                  const uint16_t   length)
{
    ...
    memcpy(data_out, header, header_length);
    memcpy(data_out + sizeof(header), (uint8_t*)(&length), sizeof(length));
    memcpy(data_out + sizeof(header) + sizeof(length), data_in, length);

    for (uint16_t i = 0 ; i < length ; i++) {
        checksum += data_in[i];
    }

    data_out[sizeof(header) + sizeof(length) + length] = checksum;

    return sizeof(header) + sizeof(length) + length + sizeof(checksum);
}
